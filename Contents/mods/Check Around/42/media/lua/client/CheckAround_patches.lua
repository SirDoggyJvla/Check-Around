...
This file handles the patches to CheckAround with other mods.
...
local CheckAroundModOptions = require "CheckAround_ModOptions"
CheckAroundModOptions = CheckAroundModOptions.options_data
require "CheckAround"
...
if activatedMods_BB_SporeZones then
    ...
    CheckAround.CheckForSporeZone = function(player,square)
        ...
        if building then
            ...
            if buildingSq then
                ...
                if isSporeZone then
                    -- retrieve voice line
                    local voiceLines = CheckAround.Voicelines_sporeZone
...
function JoypadControllerData:onPressButtonNoFocus(button)
    ...
    if player and player:isAlive() then
        ...
        local secondaryButton = secondaryButtons[CheckAroundModOptions.CheckAroundDownstairs_secondary.value]
        if not secondaryButton or secondaryButtons_functions[secondaryButton](playerIndex) then
            -- check for main key
            local mainButton = mainButtons[CheckAroundModOptions.CheckAroundDownstairs_main.value]
    ...
    CheckAround.checkForZombies(player)
end